...
WIN_SCORE = 15          # 获胜分数阈值
cheat_mode = False      # 是否开启作弊模式
cheat_probability = 0.3  # 作弊模式基础触发概率
MAX_SCORE=15
...
def display_result(frame, result_text, score, wins, losses, draws):  # 新增三个参数
    """显示游戏结果和统计数据"""
    ...
    # 3. 比赛结果
    cv2.putText(frame, result_text, (frame.shape[1]//2 - 50, 150), 
                cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 3)
    
    # 4. 统计数据（使用传入的参数）
    stats_text = f"Score: {score} | Wins: {wins} | Losses: {losses} | Draws: {draws}"
    cv2.putText(frame, stats_text, 
                (10, frame.shape[0] - 30), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    
    # 5. 操作提示
    cv2.putText(frame, "Press ENTER for next, ESC to quit",
                (10, frame.shape[0] - 10), 
...
def save_rehab_snapshot(frame, gesture, round_count, hand_bbox):
    try:
        # 1. 直接保存到桌面（无需创建子目录）
        desktop = Path.home() / "Desktop"  # 获取桌面路径
        desktop.mkdir(exist_ok=True)  # 确保桌面目录存在
        
        # 2. 生成安全文件名
        illegal_chars = r'[\\/:*?"<>|]'  
        safe_gesture = re.sub(illegal_chars, "_", gesture)
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"rehab_{round_count}_{timestamp}_{safe_gesture}.jpg"
        ...
        text_x = min(max(10, w-100), w)  # 确保在图像范围内
        text_y = min(max(30, 0), h-10)   # 确保在图像范围内
        
        cv2.putText(annotated_frame, f"Gesture: {gesture}",
                    (text_x, text_y),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)  # 正确字体
...
with mp_hands.Hands(max_num_hands=1, min_detection_confidence=0.7) as hands:
    ...
    while cap.isOpened():
        ...
        if gesture_captured:
            ...
            if player_gesture != "None":
               # 截图保存逻辑
                   save_rehab_snapshot(frame, player_gesture, round_count,hand_bbox)

                    # Step 4: 显示结果和统计信息
                   display_result(frame, result_text, score, wins, losses, draws)
    ...
    # 显示恭喜通关信息
           ...
           cap.release()
           cv2.destroyAllWindows()
           print("\n🎉 Congratulations! You've won the game with 15 points! 🎉")
           exit()
